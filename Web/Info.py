...
keyWords = {
    ...
    "设备类型": "category",
    "购买人": "Purchaser",
    "购买时间（格式：2020-01-01---2020-02-01）": 5,
}
app = Flask(__name__)
...
def submit_info():
    ...
    database = "../DataBase/articleDatabase/personQueryInfo.db"
...
def upload_file():
    if request.method == "POST":
        ...
        fileFig = file[0]
        urlFig = UPLOAD_FOLDER + request.form["ckName"] + "/" + fileFig.filename
        if len(file) > 1:
            fileFile = file[1]
            urlFile = UPLOAD_FOLDER + request.form["ckName"] + "/" + fileFile.filename
        else:
            fileFile = False
        ...
        if not os.path.exists(app.config["UPLOAD_FOLDER"] + request.form["ckName"]):
            os.makedirs(app.config["UPLOAD_FOLDER"] + request.form["ckName"])
        ...
        if fileFig and allowed_file(fileFig.filename):
            ...
            fileFig.save(
                os.path.join(
                    app.config["UPLOAD_FOLDER"] + request.form["ckName"] + "/", filename
                )
            )
    ...
    database = "../DataBase/equDatabase/equipmentInfo_" + request.form["ckName"] + ".db"
    conn = sqlite3.connect(database)
    c = conn.cursor()
    ...
    c.execute(
        ...
        (
            (
                request.form["equName"],
                request.form["equInfo"],
                request.form["category"],
                request.form["institution"],
                request.form["number"],
                request.form["Purchaser"],
                request.form["time"],
                urlFig,
                urlFile,
            )
...
def query():
    ...
    if classes == 0:
        info = query_info(ckName, classes, keywords)
    ...
    # 时间范围搜索
        ...
        info = query_info(ckName, classes, (startTime, stopTime))
...
def query_info(ckName, classes, keys):

    database = "../DataBase/equDatabase/equipmentInfo_" + ckName + ".db"
    info = []
    ...
    try:
        if classes == 0:
            ...
            c.execute(
                "select * from user_tb where equName like '%{}%' or equInfo like '%{}%'".format(
                    keys, keys
                )
            )
        elif classes == 5:
            c.execute(
                "select * from user_tb where time >= '{}' and time <= '{}'".format(
                    keys[0], keys[1]
                )
            )
        else:
            ...
            if is_contain_chinese(classes) and is_contain_chinese(keys):
                c.execute(
                    "select * from user_tb where '{}' = '{}'".format(classes, keys)
                )
